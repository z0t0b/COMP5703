...
def _integrate(t, n, _f):
    ...
    while (abs((newSimpson - oldSimpson) / newSimpson) > epsilon):
        ...
        for i in range(s+1):
            if i == s or i == 0:
                chosenNum += (_f(lowBound + (i * w), n))
                continue
            if i % 2 == 0:
                chosenNum += (4 * _f(lowBound + (i * w), n))
                continue
            if i % 2 != 0:
                chosenNum += (2 * _f(lowBound + (i * w), n))
                continue
        
        newSimpson = (w/3) * chosenNum
        s *= 2
